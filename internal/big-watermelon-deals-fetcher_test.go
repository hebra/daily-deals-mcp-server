...
func TestParseResponseJson(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			offers := parseResponseJson(resp)

			if tt.shouldParse {
				if len(offers) != len(tt.expectedOffers) {
					t.Errorf("Expected %d offers, got %d", len(tt.expectedOffers), len(offers))
					return
				}

				for i, expected := range tt.expectedOffers {
					if offers[i].ProductName != expected.ProductName {
						t.Errorf("Offer %d: expected product name '%s', got '%s'", i, expected.ProductName, offers[i].ProductName)
					}
					if offers[i].Price != expected.Price {
						t.Errorf("Offer %d: expected price '%s', got '%s'", i, expected.Price, offers[i].Price)
					}
					if offers[i].Currency != expected.Currency {
						t.Errorf("Offer %d: expected currency '%s', got '%s'", i, expected.Currency, offers[i].Currency)
					}
					if offers[i].Size != expected.Size {
						t.Errorf("Offer %d: expected size '%s', got '%s'", i, expected.Size, offers[i].Size)
					}
				}
			} else {
				if len(offers) != 0 {
					t.Errorf("Expected no offers for invalid JSON, got %d", len(offers))
				}
			}
		})
	}
...
func TestParseResponseJson_NilResponse(t *testing.T) {
	offers := parseResponseJson(nil)
	if len(offers) != 0 {
		t.Errorf("Expected no offers for nil response, got %d", len(offers))
	}
}

func TestParseResponseJson_EmptyCandidates(t *testing.T) {
	resp := &genai.GenerateContentResponse{
		Candidates: []*genai.Candidate{},
	}
	offers := parseResponseJson(resp)
	if len(offers) != 0 {
		t.Errorf("Expected no offers for empty candidates, got %d", len(offers))
	}
}
